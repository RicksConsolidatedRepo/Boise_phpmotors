...
switch ($action) {
    case 'upload':
        ...
        if($imageCheck){
        $message = '<p id="warning">An image by that name already exists.</p>';
        } elseif (empty($invId) || empty($imgName)) {
        $message = '<p id="warning">You must select a vehicle and image file for the vehicle.</p>';
        } else {
        ...
        if ($result) {
        $message = '<p id="success">The upload succeeded.</p>';
        } else {
        $message = '<p id="warning">Sorry, the upload failed.</p>';
        }
        }
        ...
        header('location: /phpmotors/uploads/');
    break;
    case 'delete':
        ...
        if ($remove) {
        $message = "<p id='success'>$filename was successfully deleted.</p>";
        } else {
        $message = "<p id='warning'>$filename was NOT deleted.</p>";
        }
